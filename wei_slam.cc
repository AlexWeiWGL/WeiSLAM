...
int main(int argc, char **argv){
    if(argc != 3){
        cerr << endl << "Usage: ./main path_to_settings path_to_sequence" << endl;
        return 1;
    }
...
void LoadMask(const string &strFilenamesMask, cv::Mat &imMask){
    ...
    int count = 0;
    cv::Mat imgLabel(imMask.rows, imMask.cols, CV_8UC3);
    while(!file_mask.eof()){
        string s;
        getline(file_mask, s);
        if(!s.empty()){
                ...
                if(tmp != 0){
                    ...
                    switch (tmp)
                    {
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            imgLabel.at<cv::Vec3b>(count, i) = cv::Vec3b(47, 255, 173);
                            break;
                        case 5:
                    ...
                    imgLabel.at<cv::Vec3b>(count, i) = cv::Vec3b(255, 255, 240);
                }
            }
            count++;
        }
