...
namespace cvplot{
    ...
    void Series::ensureDimsDepth(int dims, int depth) {
        if(dims_ != dims){
            if(dims_ != 0)
            {
                std::cerr << "incorrent dims (input dimensions), was " << dims_ << " now"
                            << dims << std::endl;
            }
            dims_ = dims;
        }
        if(depth_ != depth){
            if(depth != 0){
                cerr << "incorrect depth (output dimensions), was " << depth_
                    << " now" << depth << endl;
            }
            depth_ = depth;
        }
    ...
    Figure &Figure::sqaure(bool square) {
        aspect_sqaure_ = square;
        return *this;
    }
