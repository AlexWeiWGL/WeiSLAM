...
namespace WeiSLAM
{
    ...
    int Optimizer::PoseOptimizationNew(Frame *pCurFrame, Frame *pLastFrame, vector<int> &TemperalMatch)
    {
        ...
        vSE3->setEstimate(Converter::toSE3Quat(pCurFrame->camPose));
        vSE3->setId(0);
        vSE3->setFixed(false);
    ...
    int Optimizer::PoseOptimizationFlow2Cam(Frame *pCurFrame, Frame *pLastFrame, vector<int> &TemperalMatch)
    {
        ...
        // Set Frame vertex
        g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
        cv::Mat Init = pCurFrame->camPose; // initial with camera pose
        ...
        for(int i=0; i<N; i++)
        {
            ...
            if(mono)
            {
                ...
                const cv::Mat Rlw = pLastFrame->camPose.rowRange(0,3).colRange(0,3);
                const cv::Mat Rwl = Rlw.t();
                const cv::Mat tlw = pLastFrame->camPose.rowRange(0,3).col(3);
    ...
    cv::Mat Optimizer::PoseOptimizationObjMot(Frame *pCurFrame, Frame *pLastFrame, const vector<int> &ObjId, std::vector<int> &InlierID)
    {
        ...
        cv::Mat Init = Converter::toInvMatrix(pCurFrame->camPose)*pCurFrame->mInitModel; // initial with identity matrix
        vSE3->setEstimate(Converter::toSE3Quat(Init));
        vSE3->setId(0);
        vSE3->setFixed(false);
        optimizer.addVertex(vSE3);
        ...
        PP = KK*Converter::toMatrix4d(pCurFrame->camPose); // *Converter::toMatrix4d(Twp)
