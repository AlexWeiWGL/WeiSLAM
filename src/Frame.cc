...
namespace WeiSLAM{
    ...
    Frame::Frame(const Frame &frame)
        : imORBextractorLeft(frame.imORBextractorLeft), imORBextractorRight(frame.imORBextractorRight), timeStamp(frame.timeStamp),
        K(frame.K.clone()), distCoef(frame.distCoef.clone()), mbf(frame.mbf), mb(frame.mb), thDepth(frame.thDepth), thDepthObj(frame.thDepthObj),
        N(frame.N), mvKeys(frame.mvKeys), mvKeysRight(frame.mvKeysRight), mvKeysUh(frame.mvKeysUh), mvuRight(frame.mvuRight), mvDepth(frame.mvDepth),
        descriptor(frame.descriptor.clone()), descriptorsRight(frame.descriptorsRight.clone()), mvbOutlier(frame.mvbOutlier), mnId(frame.mnId),
        scaleFactor(frame.scaleFactor), scaleLevels(frame.scaleLevels), logScaleFactor(frame.logScaleFactor), scaleFactors(frame.scaleFactors),
        invScaleFactors(frame.invScaleFactors), invLevelSigma2(frame.invLevelSigma2), levelSigma2(frame.levelSigma2), mTcw_gt(frame.mTcw_gt),
        vObjPose_gt(frame.vObjPose_gt), nSemPosi_gt(frame.nSemPosi_gt), objLabel(frame.objLabel), nModLabel(frame.nModLabel), nSemPosition(frame.nSemPosition),
    ...
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imFlow, const cv::Mat &maskSEM,
                 ...
                 timeStamp(timeStamp), K(K.clone()), distCoef(distCoef.clone()), mbf(bf), thDepthObj(theDepthObj), thDepth(thDepth){
