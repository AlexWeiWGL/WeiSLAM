...
namespace WeiSLAM{
    ...
    float TwoViewReconstruction::CheckFundamental(const cv::Mat &F21, std::vector<bool> &vbMatchesInliers, float sigma) {
        const int N = mvMatches12.size();
    ...
    bool TwoViewReconstruction::ReconstructF(std::vector<bool> &vbMatchesInliers, cv::Mat &F21, cv::Mat &K,
        ...
        int nGood2 = CheckRT(R2, t1, mvKeys2, mvKeys2, mvMatches12, vbMatchesInliers, K, vP3D2, 4.0*mSigma2, vbTriangulated2, parallax2);
    ...
    bool TwoViewReconstruction::ReconstructH(std::vector<bool> &vbMatchesInliers, cv::Mat &H21, cv::Mat &K,
                                             cv::Mat &R21, cv::Mat &t21, std::vector<cv::Point3f> &vp3D,
                                             std::vector<bool> &vpTriangulated, float minParallax,
                                             int minTriangulated) {
        int N=0;
        for(size_t i=0, iend = vbMatchesInliers.size(); i<iend; i++)
        {
        ...
        if(secondBestGood<0.75*bestGood && bestParallax>=minParallax && bestGood>minTriangulated && bestGood>0,9*N)
        {
            ...
            vp3D = bestP3D;
            vpTriangulated = bestTriangulated;
    ...
    void TwoViewReconstruction::Normalize(const std::vector<cv::KeyPoint> &vKeys,
        ...
        for(int i=0; i<N; i++)
        {
            ...
            meanX += fabs(vNormalizedPoints[i].x);
            meanY += fabs(vNormalizedPoints[i].y);
        }
    ...
    int TwoViewReconstruction::CheckRT(const cv::Mat &R, const cv::Mat &t, const std::vector<cv::KeyPoint> &vKeys1,
                                       const std::vector<cv::KeyPoint> &vKeys2, const std::vector<Match> &vMatches12,
                                       std::vector<bool> &vbInliers, const cv::Mat &K, std::vector<cv::Point3f> &vP3D,
                                       float th12, std::vector<bool> &vbGood, float &parapllax) {
        ...
        for(size_t i=0, iend=vMatches12.size(); i<iend; i++){
            ...
            //check depth in front of first camera
            ...
            im2y = fx*p3dC2.at<float>(1)*invZ2+cy;

            float squareError2 = (im2x-kp2.pt.x)*(im2x-kp2.pt.x) + (im2y-kp2.pt.y)*(im2y-kp2.pt.y);

            if(squareError2>th12)
                continue;
        ...
        if(nGood > 0)
        {
            ...
            parapllax = acos(vCosParallax[idx])*180/CV_PI;
        }
        else
            parapllax = 0;
        return nGood;
